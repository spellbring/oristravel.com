...
class bookingModel extends Model
{
    ...
    public function getConsRes($desde, $hasta, $tipoFecha, $idAgencia, $tipoUser, $usuario)
    {
		...
		if($tipoFecha==1){ $sql.=" AND fecha BETWEEN '".$desde."' AND '".$hasta."' "; }
		elseif($tipoFecha==2){ $sql.=" AND fecha_in BETWEEN '".$desde."' AND '".$hasta."' "; }
		
		
		if($tipoFecha==1){ $sql.=" ORDER BY fecha,id_booking ASC"; }
		elseif($tipoFecha==2){ $sql.=" ORDER BY fecha_in,id_booking ASC";}
